...
public class SphereShape extends Shape implements CircleLikeShape, DrawableTriangle {
    ...
    public SphereMode mode = SphereMode.UV;
    public ArrayList<Vec3d> vertexes = new ArrayList<>();
    public Color color = Color.white;

    public SphereShape(RenderingType type, BiConsumer<SphereTransformer, Shape> transform, SphereMode mode, Vec3d center, int radius, Color color) {
        this(type, transform, mode, center, 32, radius, color, false);
    }

    public SphereShape(RenderingType type, BiConsumer<SphereTransformer, Shape> transform, SphereMode mode, Vec3d center, int segments, float radius, Color color) {
        this(type, transform, mode, center, segments, radius, color, false);
    }

    public SphereShape(RenderingType type, BiConsumer<SphereTransformer, Shape> transform, SphereMode mode, Vec3d center, int segments, float radius, Color color, boolean seeThrough) {
            ...
            case UV -> {
                ...
                for (int i = 0; i < latSegments; i++) {
                    ...
                    for (int j = 0; j < lonSegments; j++) {
                        ...
                        Vec3d v0 = sphericalToCartesian(center, radius, theta1, phi1);
                        Vec3d v1 = sphericalToCartesian(center, radius, theta1, phi2);
                        Vec3d v2 = sphericalToCartesian(center, radius, theta2, phi1);
                        Vec3d v3 = sphericalToCartesian(center, radius, theta2, phi2);
    ...
    private Vec3d sphericalToCartesian(Vec3d center, double r, double theta, double phi) {
    ...
    public static class IcoSphereGenerator {
        ...
        public IcoSphereGenerator(Vec3d center, double radius, int segments) {
            this.center = center;
            this.radius = radius;
            this.segments = segments;
            generateIcosahedron();
        ...
        private void generateIcosahedron() {
            ...
            vertices.add(normalize(new Vec3d(-1, t, 0), radius));
            vertices.add(normalize(new Vec3d(1, t, 0), radius));
            vertices.add(normalize(new Vec3d(-1, -t, 0), radius));
            vertices.add(normalize(new Vec3d(1, -t, 0), radius));

            vertices.add(normalize(new Vec3d(0, -1, t), radius));
            vertices.add(normalize(new Vec3d(0, 1, t), radius));
            vertices.add(normalize(new Vec3d(0, -1, -t), radius));
            vertices.add(normalize(new Vec3d(0, 1, -t), radius));

            vertices.add(normalize(new Vec3d(t, 0, -1), radius));
            vertices.add(normalize(new Vec3d(t, 0, 1), radius));
            vertices.add(normalize(new Vec3d(-t, 0, -1), radius));
            vertices.add(normalize(new Vec3d(-t, 0, 1), radius));
        ...
        private int getMiddlePoint(int p1, int p2) {
            ...
            Vec3d point1 = vertices.get(p1);
            Vec3d point2 = vertices.get(p2);
        ...
        private Vec3d normalize(Vec3d v, double r) {
        ...
        public ArrayList<Vec3d> getVertexList() {
