...
import ml.mypals.ryansserverrenderingkit.shape.box.WireframedBoxShape;
...
public class WireframedBoxBuilder extends BaseBuilder<WireframedBoxBuilder, BoxLikeShape.BoxTransformer>
    ...
    @Override
    public WireframedBoxBuilder min(Vec3 min) {
        this.min = min;
        return this;
    }

    @Override
    public WireframedBoxBuilder max(Vec3 max) {
        this.max = max;
        return this;
    }

    @Override
    public WireframedBoxBuilder aabb(Vec3 min, Vec3 max) {
        this.min = min;
        this.max = max;
        return this;
    }
    ...
    @Override
    public WireframedBoxBuilder size(Vec3 size) {
        this.max = this.min.add(size);
        return this;
    }

    @Override
    public WireframedBoxBuilder construction(BoxShape.BoxConstructionType type) {
        this.constructionType = type;
        return this;
    }

    public WireframedBoxBuilder edgeColor(Color edgeColor) {
        this.edgeColor = edgeColor;
        return this;
    }

    public WireframedBoxBuilder edgeWidth(float edgeWidth) {
        this.edgeWidth = edgeWidth;
        return this;
    }

    public WireframedBoxBuilder lineSeeThrough(boolean lineSeeThrough) {
        this.lineSeeThrough = lineSeeThrough;
        return this;
    }

    @Override
    public WireframedBoxShape build() {
        var t = getTransformer();
        WireframedBoxShape shape = new WireframedBoxShape(
                t, min, max,
                color,
                edgeColor,
                edgeWidth,
